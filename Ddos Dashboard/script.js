...
function setupEventListeners() {
    ...
    // Guide button
    elements.guideBtn?.addEventListener('click', openGuidePDF);
    document.getElementById('viewGuideBtn')?.addEventListener('click', openGuidePDF);
...
function openGuidePDF() {
    // This is the path to your PDF file
    const pdfPath = 'D:/CyberSecurity FYP/Ddos Dashboard/Dataset Guidelines.pdf';
    ...
    try {
        ...
        // An alternative is to host the PDF in your project folder
        const fullPath = 'file:///' + pdfPath.replace(/\\/g, '/');
        window.open(fullPath, '_blank');
        
        showToast('Opening dataset preparation guide...', 'info');
    } catch (error) {
        console.error('Error opening PDF:', error);
        
        // Fallback: Show a message with the path
        showToast(`Could not open PDF. File location: ${pdfPath}`, 'error');
        
        // Alternative: Create a download link
        const link = document.createElement('a');
        link.href = 'javascript:void(0)';
        link.onclick = function() {
            alert('To view the guide, please open this file:\n' + pdfPath);
        };
        link.textContent = 'View Guide Location';
...
document.addEventListener('DOMContentLoaded', () => {
    initDashboard();
    loadSavedTheme();
});// Open guide PDF - FIXED VERSION
function openGuidePDF() {
    // Try multiple approaches to open the PDF
    
    // Approach 1: Try relative path if PDF is in project folder
    const relativePath = './Dataset_Guidelines.pdf';
    
    // Approach 2: Your specific path
    const absolutePath = 'D:/CyberSecurity FYP/Ddos Dashboard/Dataset Guidelines.pdf';
    
    // Approach 3: Try to open with FileReader API (if PDF is small)
    try {
        // First try relative path
        window.open(relativePath, '_blank');
        showToast('Opening dataset preparation guide...', 'info');
        
        // Check if window opened successfully after 1 second
        setTimeout(() => {
            // If still showing current page, try the absolute path
            if (document.hasFocus()) {
                console.log('Relative path failed, trying absolute path...');
                
                // Convert path to proper format
                const formattedPath = 'file:///' + absolutePath.replace(/\\/g, '/').replace(/ /g, '%20');
                
                // Create an iframe as backup
                const iframe = document.createElement('iframe');
                iframe.style.display = 'none';
                iframe.src = formattedPath;
                document.body.appendChild(iframe);
                
                // Show alternative options
                setTimeout(() => {
                    if (document.hasFocus()) {
                        showToast('Could not open PDF automatically. Please open it manually.', 'warning');
                        
                        // Show manual options
                        const userConfirmed = confirm(
                            'Unable to open PDF automatically.\n\n' +
                            'Please do one of the following:\n\n' +
                            '1. Move the PDF to your project folder as "Dataset_Guidelines.pdf"\n' +
                            '2. Open it manually from:\n' +
                            'D:\\CyberSecurity FYP\\Ddos Dashboard\\Dataset Guidelines.pdf\n\n' +
                            'Click OK to copy the path to clipboard.'
                        );
                        
                        if (userConfirmed) {
                            navigator.clipboard.writeText('D:\\CyberSecurity FYP\\Ddos Dashboard\\Dataset Guidelines.pdf')
                                .then(() => showToast('Path copied to clipboard!', 'success'))
                                .catch(() => showToast('Path: D:\\CyberSecurity FYP\\Ddos Dashboard\\Dataset Guidelines.pdf', 'info'));
                        }
                    }
                }, 1000);
            }
        ...
        showToast('Unable to open PDF. Please check file location.', 'error');
        ...
        link.onclick = function() {
            alert(
                ...
                'D:\\CyberSecurity FYP\\Ddos Dashboard\\Dataset Guidelines.pdf\n\n' +
                'Solutions:\n' +
                '1. Move the PDF to your project folder\n' +
                '2. Rename it to "Dataset_Guidelines.pdf"\n' +
                '3. Ensure no spaces in filename\n' +
                '4. Open it manually from the above path'
            );
        };
        link.textContent = 'View Guide Location';
        document.body.appendChild(link);
    ...
    try {
        ...
        showToast('Opening guide...', 'info');
    } catch (error) {
        // If it fails, show the modal with instructions
